...
`--output` is optional (default: `results.tsv`). `biocol run` also writes BLAST tabular files to `--blast-dir` (default: `blast/` next to the TSV). Optional `--hmm-db Pfam-A.hmm` runs hmmscan (protein query; `hmmpress` if needed) and writes `hmm/hmmscan.tbl`. Help text and errors are in English.
...
Errores: `FastaError`, `EmptyFastaError`, `InvalidFastaError`, `MixedSequenceTypeError`, `BlastError`, `DatabaseError`, `MixedDatabaseTypeError`, `BlastExecutionError`, `HmmError`, `HmmExecutionError`, `MetadataError`.
